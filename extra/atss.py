...
class ATSS(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        # if cfg.MODEL.ATSS.get('USE_COSSIM', False):
        #     head = ATSSHeadCosSim(cfg, feature_shapes)
        # else:
    ...
    def _save_targets_on_imgs_for_debug(self, images, batched_inputs):
        # using cv2, depict gt bounding boxes on images and save them
        import cv2
        for ind, (i,b) in enumerate(zip(images, batched_inputs)):
            t = b['instances']
            img = i.cpu().numpy().transpose(1,2,0)
            img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)*255
            boxes = t.get('gt_boxes')
            labels = t.get('gt_classes')
            for (box,label) in zip(boxes, labels):
                x1,y1,x2,y2 = box.cpu().numpy().astype(int)
                img = cv2.rectangle(img, (x1,y1), (x2,y2), (0,255,0), 1)
                # add label name at the top of bbox
                label = label.cpu().numpy()
                img = cv2.putText(img, str(label), (x1,y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1)
            cv2.imwrite('debug/trainloop_{}.jpg'.format(str(ind).zfill(2)), img)
        return
    ...
    def forward(self, batched_inputs: List[Dict[str, Tensor]]):
        ...
        # [debug]
        # self._save_targets_on_imgs_for_debug(images.tensor, batched_inputs)
        features = self.backbone(images.tensor)
        features = [features[f] for f in self.head_in_features]

        anchors = self.anchor_generator(features) # one anchor per one grid
        ...
        if self.training:
            ...
            # gt_labels, gt_boxes, _flag = self.label_anchors(anchors, gt_instances)
            # if _flag:
            #     from detectron2.utils import comm
            #     rank = comm.get_rank()
            #     print('rank: {} | len(batch)={}'.format(rank, len(batched_inputs)))
            #     print('rank: {} | file_names={}'.format(rank, [b['file_name'] for b in batched_inputs]))
    ...
    def label_anchors(self, anchors, targets):
        cls_labels = []
        reg_targets = []
        for im_i in range(len(targets)): # per batch
            ...
            for level, anchors_per_level in enumerate(anchors):
                ...
                _, topk_idxs_per_level = distances_per_level.topk(topk, dim=0, largest=False) # smallest topk anchors
            ...
            # Limiting the final positive samples’ center to object (if center points of anchors are in the gt boxes or not (with margin 0.01))
            ...
            # # [debug]
            # try:
            #     _l = e_anchors_cx[candidate_idxs].view(-1, num_gt) - bboxes_per_im[:, 0]
            #     _t = e_anchors_cy[candidate_idxs].view(-1, num_gt) - bboxes_per_im[:, 1]
            #     _r = bboxes_per_im[:, 2] - e_anchors_cx[candidate_idxs].view(-1, num_gt)
            #     _b = bboxes_per_im[:, 3] - e_anchors_cy[candidate_idxs].view(-1, num_gt)
            #     _flag = False
            # except:
...
# NOTE: As ATSS employ FocalSigmoid classifier, we do not use CosineSimilarityOutputLayers.
# The followings are not used in this implementation.
 
# class ATSSHeadCosSim(ATSSHead):
#     def __init__(self, cfg, input_shape: List[ShapeSpec]):
#         super(ATSSHeadCosSim, self).__init__(cfg, input_shape)
#         self.sim_scale = cfg.MODEL.ROI_HEADS.COSINE_SCALE
#         if self.sim_scale == -1:
#             # learnable global scaling factor
#             self.sim_scale = nn.Parameter(torch.ones(1) * 20.0)
...
#             # normalize inputs
#             x_norm = torch.norm(cls_tower, p=2, dim=1).unsqueeze(1).expand_as(cls_tower)
#             x_normalized = cls_tower.div(x_norm + 1e-5)
#             # normalize weight
#             temp_norm = (
#                 torch.norm(self.cls_logits.weight.data, p=2, dim=1).unsqueeze(1).expand_as(self.cls_logits.weight.data)
#             )
#             self.cls_logits.weight.data = self.cls_logits.weight.data.div(temp_norm + 1e-5)
#             cos_dist = self.cls_logits(x_normalized)
#             scores = self.sim_scale * cos_dist
#             cos_dists.append(scores)

#             bbox_pred = self.scales[l](self.bbox_pred(box_tower))
#             bbox_reg.append(bbox_pred)

#             centerness.append(self.centerness(box_tower))
#         return cos_dists, bbox_reg, centerness